...
## 資料型別 (`types.ts`)
- **`AIAnalysis`**：`summary`(字串)、`strengths`(字串陣列)、`growthAreas`(字串陣列)、`actionPlan`(字串陣列)。
...
2. **評鑑週期切換**：
   - 頁面頂端有下拉選單，可切換不同 Nomination（顯示 `nomination.title`），預設選取第一筆。
   - 切換後所有數據面板即時更新。
...
3. **視覺化數據 — 落差分析雷達圖 (SVG Radar Chart)**：
   ...
   - **計算邏輯**：每個維度取該維度下所有題目的平均分數。他評平均排除 `score === 1`（代表「不清楚」），自評則不排除。
...
4. **Gemini AI 成長導師面板**：
   ...
   - 點擊「生成 AI 洞察」按鈕觸發分析。
   - **前提條件**：需包含自評與至少一位他評（`currentFeedbacks.length >= 2`），不足則彈出 Alert 提示。
   - **AI 服務 (`geminiService.ts`)**：
     ...
     - 回傳 JSON 結構：`summary`（≈60 字摘要）、`strengths`（3 項優勢）、`growthAreas`（2 項改進建議）、`actionPlan`（3 項行動步驟）。
